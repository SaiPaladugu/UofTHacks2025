...
import type { LocationBoardDerivedState, LocationBoardContract, LocationBoardProviders, DeployedLocationBoardContract } from './common-types.js';
import {
  type LocationBoardPrivateState,
  Contract,
  createLocationBoardPrivateState,
  ledger,
  pureCircuits,
  witnesses,
  STATE,
...
/** @internal */
const locationBoardContractInstance: LocationBoardContract = new Contract(witnesses);

/**
 * An API for a deployed bulletin board.
 */
export interface DeployedLocationBoardAPI {
  readonly deployedContractAddress: ContractAddress;
  readonly state$: Observable<LocationBoardDerivedState>;
...
export class LocationBoardAPI implements DeployedLocationBoardAPI {
  /** @internal */
  private constructor(
    public readonly deployedContract: DeployedLocationBoardContract,
    ...
    this.state$ = combineLatest(
      [
        ...
        from(providers.privateStateProvider.get('locationBoardPrivateState') as Promise<LocationBoardPrivateState>),
      ],
   ...
   */
  static async deploy(providers: LocationBoardProviders, logger?: Logger): Promise<LocationBoardAPI> {
    logger?.info('deployContract');

    // EXERCISE 5: FILL IN THE CORRECT ARGUMENTS TO deployContract
    const deployedLocationBoardContract = await deployContract(providers, {
      // EXERCISE ANSWER
      privateStateKey: 'locationBoardPrivateState', // EXERCISE ANSWER
      contract: locationBoardContractInstance,
      initialPrivateState: await LocationBoardAPI.getPrivateState(providers), // EXERCISE ANSWER
    });

    logger?.trace({
      contractDeployed: {
        finalizedDeployTxData: deployedLocationBoardContract.deployTxData.public,
      },
    });

    return new LocationBoardAPI(deployedLocationBoardContract, providers, logger);
  }
   ...
   */
  static async join(providers: LocationBoardProviders, contractAddress: ContractAddress, logger?: Logger): Promise<BBoardAPI> {
    logger?.info({
      joinContract: {
        contractAddress,
      },
    ...
    const deployedBBoardContract = await findDeployedContract(providers, {
      contractAddress,
      contract: locationBoardContractInstance,
      privateStateKey: 'locationBoardPrivateState',
      initialPrivateState: await LocationBoardAPI.getPrivateState(providers),
    });
    ...
    return new LocationBoardAPI(deployedBBoardContract, providers, logger);
  }

  private static async getPrivateState(providers: LocationBoardProviders): Promise<LocationBoardPrivateState> {
    const existingPrivateState = await providers.privateStateProvider.get('locationBoardPrivateState');
    return existingPrivateState ?? createLocationBoardPrivateState(utils.randomBytes(32));
  }
}
